...
import rasterio
from cempa.config import settings, lats, lons, logger
...
def nc2tiff(name, var, coll_name, file_name):
    ...
    if isfile(name_tif) and isfile(f"{name_map}") and not settings.forceCreateFiles:
        logger.info("Tiff e map ja foi gerado")
        return None
    start_time = time()
    logger.info(f"Criando tiff /{get_time(file_name,True)}/{name}/{coll_name}.tif")
    ...
    min_max = (round(np.nanmin(tmp_var), 2), round(np.nanmax(tmp_var) + 0.05, 2))
